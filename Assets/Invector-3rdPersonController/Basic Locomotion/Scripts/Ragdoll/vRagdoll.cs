...
{
    [vClassHeader("RAGDOLL SYSTEM", true, "ragdollIcon", true, "Every gameobject children of the _character must have their tag added in the IgnoreTag List.")]
    public class vRagdoll : vMonoBehaviour
    {
        ...
        {
            bodyParts.Clear();
            // find _character chest and hips
            if (!animator)
            {
            ...
            // find all the transforms in the _character, assuming that this script is attached to the root
            if (characterHips)
            {
        ...
        // reset player - restore control to the _character	
        ...
        void RagdollBehaviour()
        {
            ...
            if (state == RagdollState.blendToAnim)
            {
                if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
                {
                    ...
                    //_character to the best match with the ragdoll
                    ...
                    //Now cast a ray from the computed position downwards and find the highest hit that does not belong to the _character 
                ...
                foreach (BodyPart b in bodyParts)
                {
                    ...
                    { //this if is to prevent us from modifying the root of the _character, only the actual body parts
        ...
        // destroy the components if the _character is dead
